...
class Devices:
    ...
    async def list_devices(self, callback_query: CallbackQuery, state: FSMContext):
        """
        ...
        keyboard.add(InlineKeyboardButton("Добавить устройство",
                                          callback_data=f"/create_device_<id:{fz_user_id}>"))
    ...
    def _fill_buttons_for_list_devices(self, devices: list[Device], keyboard: InlineKeyboardMarkup):
        ...
        for i, device in enumerate(devices):
            ...
            row.append(InlineKeyboardButton(button_text,
                                            callback_data=f"/{prefix}_<id:{device.id}>"))
    ...
    async def device_details(self, callback_query: CallbackQuery, state: FSMContext):
        ...
        keyboard.add(InlineKeyboardButton("Удалить",
                                          callback_data=f"/{self.__class__.device_confirm_delete_prefix}<id:{device_id}>"))
    ...
    async def device_confirm_delete(self, callback_query: CallbackQuery, state: FSMContext):
        ...
        keyboard.add(InlineKeyboardButton("Да",
                                          callback_data=f"/{delete_device_prefix}<id:{device_id}>"),
                     InlineKeyboardButton("Отмена",
                                          callback_data=f"/{device_details_prefix}<id:{device_id}>_<edit>")
                     )
    ...
    async def delete_device(self, callback_query: CallbackQuery, state: FSMContext):
        # TODO: реализовать
        raise NotImplementedError("Необходимо реализовать диалог")
