...
def prepare_configuration_qr_and_message(device: Device, file_name: str = "EuroHoster.conf") -> tuple[StringIO, BytesIO]:
    """
...
def prepare_config_file(device: Device, file_name: str) -> tuple[StringIO, str]:
    ...
    config['Interface'] = {"PrivateKey": device.private_key,
                           "Address": address,
                           "MTU": device.mtu,
                           "DNS": ','.join(device.dns)}
    config["Peer"] = {"PresharedKey": device.preshared_key,
